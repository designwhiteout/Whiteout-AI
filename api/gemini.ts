...
// To use it, create a file at `/api/gemini.ts` in your project root.
// Vercel automatically deploys any file in the `/api` directory as a serverless function.
// Ensure you have `@google/genai` in your package.json dependencies.
// Set your API_KEY as an environment variable in your Vercel project settings.

import { GoogleGenAI, Modality, Part, Type } from "@google/genai";
import type { ComparativeCostItem } from '../types';

// This function can be named anything, but `handler` is a common convention.
// The `export default` is required for Vercel to recognize it.
...
Surfaces should feature micro-imperfections: variations in wood grain, fabric folds, stone veining, minor wear—underscoring authenticity.`;
  ...
  try {
    ...
    switch (action) {
      case 'generateDesign':
        result = await generateDesignInternal(ai, SYSTEM_INSTRUCTION, payload);
        break;
      case 'adjustDesign':
        result = await adjustDesignInternal(ai, SYSTEM_INSTRUCTION, payload);
        break;
      case 'estimateCost':
        result = await estimateCostInternal(ai, payload);
        break;
      default:
...
const generateDesignInternal = async (ai: GoogleGenAI, systemInstruction: string, {
  base64ImageData,
  mimeType,
  ...
  referenceMimeType,
    ...
    if (base64ReferenceImageData && referenceMimeType) {
      parts.push({ inlineData: { data: base64ReferenceImageData, mimeType: referenceMimeType } });
    ...
    const response = await ai.models.generateContent({
      ...
      contents: { parts },
      config: { systemInstruction, responseModalities: [Modality.IMAGE, Modality.TEXT] },
    });
...
const adjustDesignInternal = async (ai: GoogleGenAI, systemInstruction: string, {
  base64ImageData,
  mimeType,
  adjustmentPrompt,
  ...
  if (!adjustmentPrompt.trim()) throw new Error("Adjustment instructions cannot be empty.");
  ...
  if (base64AdjustmentImageData && adjustmentMimeType) {
    parts.push({ inlineData: { data: base64AdjustmentImageData, mimeType: adjustmentMimeType } });
...
const estimateCostInternal = async (ai: GoogleGenAI, { base64ImageData, mimeType }: { base64ImageData: string, mimeType: string }): Promise<ComparativeCostItem[]> => {
      ...
      config: {
        ...
        responseSchema: {
          ...
          items: {
            type: Type.OBJECT,
            properties: {
              item: { type: Type.STRING, description: 'The general category of the item (e.g., "Kitchen Benchtop", "Sofa").' },
              boundingBox: { type: Type.ARRAY, description: "A normalized bounding box for the item's location in the image, in the format [yMin, xMin, yMax, xMax].", items: { type: Type.NUMBER } },
              options: {
                type: Type.ARRAY,
                ...
                items: {
                  type: Type.OBJECT,
                  properties: {
                    optionName: { type: Type.STRING, description: 'The specific name for this option (e.g., "Mid-Range Fabric Sofa").' },
                    description: { type: Type.STRING, description: 'A brief description of this option (e.g., "A comfortable 3-seater sofa upholstered in a durable polyester-blend fabric.").' },
                    estimatedCostAUD: { type: Type.NUMBER, description: 'The estimated cost of this option in Australian Dollars (AUD).' },
                    suggestedSupplier: { type: Type.STRING, description: 'A plausible Australian supplier for this type of item (e.g., "Temple & Webster", "King Living").' }
                  },
